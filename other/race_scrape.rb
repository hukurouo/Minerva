  ...
  table.each_with_index do |tr, index|
    ...
    tr.css('td').each_with_index do |td, index|
      if [1,2,3,4,5,6,7,10,11,12,13].include?(index)
        ...
        if td.text.strip.include?("降") || td.text.strip.include?("中") || td.text.strip.include?("失") || td.text.strip.include?("取")|| td.text.strip.include?("除")
          tds.push(99)
        else
          tds.push(td.text.strip)
        end
      elsif index == 14
        if td.text.strip.include?("計不") 
          tds.push(-99)
          tds.push(-99)
        elsif td.text.strip.include?("(") 
