...
def get_result(id,agent)
  ...
  table.each_with_index do |tr, index|
    ...
    (5..9).each do |i|
      ...
      horse_name =tr.css('td')[3].css('a')[0].text.strip
      ranks << rank.to_f
    ...
    map[horse_id] = {rank: rank_sum.round(3), tyakusa: tyakusa_sum.round(3), name: horse_name}
  end
  map
end
...
def today()
  ...
  race_ids.each_with_index do |id, index|
    ...
    result = ["horseName,horseId,oikiri,comment,timePointTop,timePointWide,timePointTotal,rankPoint,tyakusaPoint".split(",")]
...
def write_today(race_result)
  CSV.open("datas/#{@dir_name}/today_data.csv", "w") do |csv| 
